...
module.exports = (src, W, H, is_slim, overlay) => {
    return loadImage(src).then(async image => {
        ...
        const canvas = createCanvas(1024, (1024 / (image.height === 64 ? 1 : 2)));
        ...
        const left_arm              = await getPart(bufferedCanvas, 704, 832, (is_slim ? 32 : 64), 199, false);
        const left_arm_overlay      = await getPart(bufferedCanvas, 832, 832, (is_slim ? 32 : 64), 199, false);
        const left_leg              = await getPart(bufferedCanvas, 64, 320, 64, 199, false);
        const left_leg_overlay      = await getPart(bufferedCanvas, 64, 576, 64, 199, false);
        const right_leg             = await getPart(bufferedCanvas, 320, 832, 64, 199, false);
        const right_leg_overlay     = await getPart(bufferedCanvas, 64, 832, 64, 199, false);
        ...
        return resize(all.toBuffer(), (W - 255), H)
    });
}
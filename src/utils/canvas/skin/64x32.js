...
module.exports = async (src, W, H, is_slim, overlay) => {
    return loadImage(src).then(async image => {
        ...
        const left_arm       = await getPart(bufferedCanvas, 704, 320, (is_slim ? 32 : 64), 199, false);
        const left_leg       = await getPart(bufferedCanvas, 64, 320, 64, 199, false);
        const right_leg      = await getPart(bufferedCanvas, 64, 320, 64, 199, false);

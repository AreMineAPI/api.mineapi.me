...
passport.use(new XboxStrategy(config.xbox,
...
router.get("/auth/minecraft", (req, res, next) => {
...
router.get('/auth/minecraft/callback', passport.authenticate('minecraft', {
    failureRedirect: '/',
    session: false,
    ...
    if (account) {
        const isClaimed = await Usernames.findOne({ id: req.user?.id }).select("claimed");
        ...
        if (isClaimed?.claimed || account?.claimed !==  null) {
            ...
            if (username) {
                ...
                const newToken = encoder({ value: `${account.id},${account.username},${account.email},${Date.now()}`, length: 64 });
                ...
                res.redirect(config.websiteUrl + '/auth/success?propmt=none&token=' + newToken + '&next=' + next);
            } else {
                res.redirect(config.websiteUrl + '/');
            }
        }

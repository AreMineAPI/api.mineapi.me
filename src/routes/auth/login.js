...
router.post('/auth/login', async (req, res) => {
    ...
    if (!username || !password) return res.json({
        success: false,
        errorId: "unknown",
        message: `${isEmail ? 'Email' : 'Username'} and password are required.`
    });
    ...
    try {
        if (!account) return res.json({
            success: false,
            errorId: "unknown",
            message: `${isEmail ? 'Email' : 'Username'} or password is incorrect.`
        });
        if (account.password !== sha256(password)) return res.json({
            success: false,
            errorId: "unknown",
            message: `${isEmail ? 'Email' : 'Username'} or password is incorrect.`
        });

...
router.post('/auth/register', (req, res) => {
    ...
    if (username.length < 3 || username.length > 32) return res.json({
        success: false,
        errorId: "username",
    ...
    if (password.length < 8 || password.length > 32) return res.json({
        success: false,
        errorId: "password",
    ...
    const token = encoder({ value: `${accId},${username},${email},${Date.now()}`, length: 64 });

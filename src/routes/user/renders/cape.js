...
router.get('/cape/:value', async (req, res) => {
    ...
    if (!['optifine', 'minecraftcapes', 'mojang'].includes(type)) return res.status(400).json({
        success: false,
    ...
    const image = cape?.data?.[type] ? (type === "mojang" ? ('http://textures.minecraft.net/texture/' + cape?.data?.[type]) : cape?.data?.[type]) : cape?.data?.optifine;
    ...
    }).catch(err => console.log(`/cape/${req.params.value} Error: ${err.message}`));
});

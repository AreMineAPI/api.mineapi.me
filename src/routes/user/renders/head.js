...
router.get('/head/:value', async (req, res) => {
    ...
    axios.get(image, {
        ...
        const HeadImage = await head(response.data, Number(width), Number(height), overlay === "true" ? true : false);
        res.end(HeadImage);
    }).catch(err => console.log(`/head/${req.params.value} Error: ${err.message}`));
});

...
router.get("/all/skins", async (req, res) => {
    ...
    setTimeout(
        async () => {
            ...
            if (!["downloads", "updatedAt", "createdAt"].includes(sort))
                return res.status(400).json({
                    success: false,
                    message:
                        "Sort must be one of these: downloads, updatedAt, createdAt.",
                });
            ...
            const total = await Skins.countDocuments({
...
router.get("/all/servers", async (req, res) => {
    ...
    if (!["onlinePlayers", "updatedAt", "createdAt"].includes(sort))
        return res.status(400).json({
            success: false,
            message:
                "Sort must be one of these: updatedAt, onlinePlayers, createdAt.",
        });
    ...
    const total = await Servers.countDocuments({
        name: { $regex: q, $options: "i" },
    });
